...
<div style="margin-top:30px">
    <form class="form-horizontal common-form" id="form-save" method="post" action="/user/grade/save" onsubmit="return false;" autocomplete="off">
    ...
    <input type=hidden value="{{$row['cust_lv_type']}}" name="cust_lv_type" id="cust_lv_type">
        ...
        <div class="form-group row form-item">
            <label style=" font-weight: normal" for="c-mch_name" class="control-label col-xs-12 col-sm-2">
                <span style="color:red">*</span> 等级积分：</label>
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="cust_lv_score" class="form-control col-sm-5" name="cust_lv_score" type="text" value="{{$row['cust_lv_score']}}" placeholder="" data-rule="等级积分:required">
                    <span class="col-sm-7 color-6A6969"> 等级积分,当会员等级达到此值时，会员就可达到该等级。最低为0分</span>
                </div>
                <span class="msg-box" style="position:static;" for="cust_lv_score"></span>
            </div>
        </div>
        <div class="form-group row form-item">
            <label style=" font-weight: normal" for="c-mch_name" class="control-label col-xs-12 col-sm-2">

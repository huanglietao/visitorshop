...
<div style="margin-top:30px">
    <form class="form-horizontal common-form" id="form-save" method="post" action="/agent/apply/save" onsubmit="return false;" autocomplete="off">
    <input type=hidden value="{{$row['agent_apply_id']}}" name="agent_apply_id" id="agent_apply_id">
    <input type=hidden value="{{$row['mch_id']}}" name="mch_id" id="mch_id">
    <input type=hidden value="{{$row['inviter_id']}}" name="inviter_id" id="inviter_id">
    <input type=hidden value="{{$row['inviter_code']}}" name="inviter_code" id="inviter_code">
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <select id="cust_lv_id" class="form-control col-sm-5" name="cust_lv_id" type="text" data-rule="店铺等级:required">
        ...
        <div class="form-group row form-item">
            <label style=" font-weight: normal" for="c-mch_name" class="control-label col-xs-12 col-sm-2">
                <span style="color:red">*</span> 审核状态：</label>
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <div style="display: inline-block;padding-top: 5px;padding-left: 0" class="col-sm-5">
                        @component('component/radio',['radio'=>['1'=>'等待审核','2'=>'审核通过','3'=>'审核不通过'],'name'=>'review_status','default_key'=>$row['review_status']??1])
                        @endcomponent
                    </div>
                    <span class="col-sm-7 color-6A6969" style="padding-top: 1%"> 审核通过即可创建分销商账号信息，审核不通过时请说明原因。</span>
                </div>
                <span class="msg-box" style="position:static;" for="review_status"></span>
            </div>
        </div>
        ...
        <div class="form-group row form-item">
            <label style=" font-weight: normal" for="c-mch_name" class="control-label col-xs-12 col-sm-2">审核不通过原因：</label>
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <textarea id="review_failed_msg" rows="10" class="form-control col-sm-5" name="review_failed_msg" type="text" placeholder="" >{{$row['review_failed_msg']}} </textarea>
                     <span class="col-sm-7 color-6A6969"> 审核不通过的原因，不超过200个字。</span>
                </div>
                <span class="msg-box" style="position:static;" for="review_failed_msg"></span>
            </div>
        </div>
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <textarea id="agent_apply_desc" rows="10" class="form-control col-sm-5" name="agent_apply_desc" type="text" placeholder="" >{{$row['agent_apply_desc']}} </textarea>

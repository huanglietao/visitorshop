...
<div style="margin-top:30px">
    <form class="form-horizontal common-form" id="form-save" method="post" action="/auth/cmsauthgroup/save" onsubmit="return false;" autocomplete="off">
        <input type=hidden value="{{$row['cms_group_id']}}" name="id" id="id">
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="cms_group_name" class="form-control col-sm-5" name="cms_group_name" type="text" value="{{$row['cms_group_name']}}" placeholder="" data-rule="角色名称:required">
                    <span class="col-sm-7 color-6A6969"> 支持中英文,长度2~15位。</span>
                </div>
                <span class="msg-box" style="position:static;" for="cms_group_name"></span>
            </div>
        </div>
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row radios">
                    <div style="display: inline-block;padding-top: 5px;padding-left: 0" class="col-sm-5">
                        @component('component/radio',['radio'=>[1=>'启用',0=>'禁用'],'name'=>'cms_group_status','default_key'=>$row['cms_group_status']??1])
                        @endcomponent
                    </div>
                    <span class="col-sm-7 color-6A6969"> 启用：正常使用；禁用：隐藏账号不能使用。</span>
                </div>
                <span class="msg-box" style="position:static;" for="cms_group_status"></span>
            </div>
        </div>

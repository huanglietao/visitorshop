...
<div style="margin-top:30px">
    <form class="form-horizontal common-form" id="form-save" method="post" action="/queue/opqueue/save" onsubmit="return false;" autocomplete="off">

        <input type=hidden value="{{$row['produce_queue_id']}}" name="id" id="id">
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <select  class="form-control col-sm-7" id="produce_queue_status" name="produce_queue_status" data-rule="队列状态:required">
                        @foreach($queueStatusList as $k=>$v)
                            <option value={{$k}} @if($k == $row['produce_queue_status']) selected @endif >{!! $v !!}</option>
                        @endforeach
                    </select>
                    <span class="col-sm-5 color-6A6969"></span>
                </div>
                <span class="msg-box" style="position:static;" for="produce_queue_status"></span>
            </div>
        </div>
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="produce_queue_err_msg" class="form-control col-sm-7" name="produce_queue_err_msg" type="text" value="{{$row['produce_queue_err_msg']}}">
                    <span class="col-sm-5 color-6A6969"> </span>
                </div>

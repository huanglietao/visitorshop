...
<div style="margin-top:30px" >
    <form class="form-horizontal common-form" id="form-save" method="post" action="/templatecenter/innerchild/save" onsubmit="return false;" autocomplete="off">
        <input type=hidden value="{{$row['inner_page_id']}}" name="id" id="id">
        <input id="specifications_id"  class="form-control" name="specifications_id" type="hidden" value="{{$specId}}">
        <input id="inner_page_tid"  class="form-control" name="inner_page_tid" type="hidden" value="{{$tid}}">
        <input id="spec_info_id"  class="form-control" name="spec_info_id" type="hidden" value="{{$specInfo['size_info_id']}}">
        <input id="inner_page_dpi"  class="form-control" name="inner_page_dpi" type="hidden" value="{{$specInfo['size_info_dpi']}}">

        <div class="form-group row form-item">
            <label style=" font-weight: normal" for="inner_page_name" class="control-label col-xs-12 col-sm-2">
            <span style="color:red">*</span> 子页名称：</label>
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="inner_page_name" class="form-control col-sm-7" name="inner_page_name" type="text" value="{{$row['inner_page_name']}}" data-rule="子页名称:required">
                    <span class="col-sm-5 color-6A6969"> 支持中英文，长度不超过30字符。</span>
                </div>
                <span class="msg-box" style="position:static;" for="inner_page_name"></span>
            </div>
        </div>
        ...
        @if($goodsCate->cate_flag==$tgl)
            <div class="form-group row form-item">
                ...
                <div class="col-xs-12 col-sm-10">
                    <div class="row">
                        <select  class="form-control col-sm-7" id="inner_page_year" name="inner_page_year" >
                            @foreach($yearList as $k=>$v)
                                <option value={{$k}} @if($k == $row['inner_page_year']) selected @endif >{!! $v !!}</option>
                            @endforeach
                        </select>
                        <span class="col-sm-5 color-6A6969"> 选择年份</span>
                    </div>
                    <span class="msg-box" style="position:static;" for="inner_page_year"></span>
                </div>
            </div>
        @endif
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="inner_page_sort" class="form-control col-sm-7" name="inner_page_sort" type="text" value="{{$row['inner_page_sort']}}" data-rule="integer(+0);length(~5)">
                    <span class="col-sm-5 color-6A6969"> 填写正整数，如：1</span>
                </div>
                <span class="msg-box" style="position:static;" for="inner_page_sort"></span>
            </div>
        </div>
        ...
        <div class="form-group row form-item">
            ...
            <div class="spec-table temp-page-spec">
                ...
                <table  class="table table-striped page-table">
                    ...
                    <tr>
                        <td style="width:80px;" align="right">单/双页：</td>

...
<!-- form表单视图 -->
<!-- 面包屑组件start  -->
@component('component/crumb',['icon' => 'fa-dashboard', 'title' => '模板中心/内页模板库' ])
@endcomponent
<!--  提示组件 start -->

<div id="main">
@component('component/tips')
    <p>内页模板是供设计人员选择生成主模板，设计人员通过添加背景、素材及相应布局组成内页模板</p>
@endcomponent


<!-- form表单视图 -->
    <div style="margin-top:30px" class="templayui-layer">
    <form class="form-horizontal common-form" id="form-save" method="post" action="/templatecenter/inner/save" onsubmit="return false;" autocomplete="off">

        <input type=hidden value="{{$row['inner_temp_id']}}" name="id" id="id">
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="inner_temp_name" class="form-control col-sm-7" name="inner_temp_name" type="text" value="{{$row['inner_temp_name']}}" placeholder="" data-rule="模板名称:required">
                    <span class="col-sm-5 color-6A6969"> 支持中英文，长度不超过30字符</span>
                </div>
                <span class="msg-box" style="position:static;" for="inner_temp_name"></span>
            </div>
        </div>
        <div class="form-group row form-item">
            <label style=" font-weight: normal" for="c-mch_name" class="control-label col-xs-12 col-sm-2">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <select  class="form-control col-sm-7" id="goods_type_id" name="goods_type_id" @if(!$row['inner_temp_id']) data-rule="商品分类:required" @endif  @if($row['inner_temp_id']) disabled @endif >
                        @foreach($goodsCateType as $k=>$v)
                            <option data-flag="{{$goodsFlag[$k]}}" value={{$k}} @if($k == $row['goods_type_id']) selected @endif >{!! $v !!}</option>
                        @endforeach
                    </select>
                    @if($row['inner_temp_id'])
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <select  class="form-control col-sm-6" id="specifications_id" name="specifications_id" @if(!$row['inner_temp_id'])data-rule="产品规格:required" @endif  @if($row['inner_temp_id']) disabled @endif >
                    ...
                    @endif
                    <a id="spec-params" class="btn btn-default btn-dialog" data-title = "查看规格参数" data-area="['45%', '65%']" data-url="@if(!$row['inner_temp_id']){{URL::asset('/templatecenter/main/specdetail')}}?id={{$default_spec[0]}} @else{{URL::asset('/templatecenter/main/specdetail')}}?id={{$row['specifications_id']}}@endif" title="规格参数"  type="button"  >规格参数</a>
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <select  class="form-control col-sm-7" id="inner_temp_theme_id" name="inner_temp_theme_id" data-rule="模板分类:required">
                        <option value= "">请选择</option>
                        @foreach($tempThemeList as $k=>$v)
                            <option value={{$k}} @if($k == $row['inner_temp_theme_id']) selected @endif >{!! $v !!}</option>
                        @endforeach
                    </select>
                    <span class="col-sm-5 color-6A6969"> 选择模板分类</span>
                </div>
                <span class="msg-box" style="position:static;" for="inner_temp_theme_id"></span>
            </div>
        </div>
        ...
        <div class="form-group row form-item" id="calendar-year" style="@if($cateFlag==$tgl) display:flex @else display:none @endif">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <select  class="form-control col-sm-7" id="inner_temp_start_year" name="inner_temp_start_year" >
                        <option value= "">请选择</option>
                        @foreach($yearList as $k=>$v)
                            <option  value={{$k}} @if($k == $row['inner_temp_start_year']) selected @endif >{!! $v !!}</option>
                        @endforeach
                    </select>
                    <span class="col-sm-5 color-6A6969"> 选择起始年份</span>
                </div>
                <span class="msg-box" style="position:static;" for="inner_temp_start_year"></span>
            </div>
        </div>
        ...
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="inner_temp_sort" class="form-control col-sm-7" name="inner_temp_sort" type="text" value="{{$row['inner_temp_sort']}}" data-rule="integer(+0);length(~5)">
                    <span class="col-sm-5 color-6A6969"> 填写正整数，如：1</span>
                </div>
                <span class="msg-box" style="position:static;" for="inner_temp_sort"></span>
            </div>
        </div>
        <div class="form-group row form-item">
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    <input  id="inner_temp_desc" class="form-control col-sm-7" name="inner_temp_desc" type="text" value="{{$row['inner_temp_desc']}}" data-rule="length(~100)">
                    <span class="col-sm-5 color-6A6969"> 支持中英文</span>
                </div>
                <span class="msg-box" style="position:static;" for="inner_temp_desc"></span>
            </div>
        </div>
        ...
        <div class="form-group row form-item" >
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    @component('component/imgUpload',['plid'=>"thumbImg",'parms'=>'thumb','name'=>'inner_temp_thumb','uploader'=>'thumbUpoad','direction'=>1,'browse_btn'=>'btn-thumb','content_class'=>'innerThumb','img_format'=>'gif,jpg,jpeg,png','num'=>1,'value'=>$row['inner_temp_thumb']])
                    @endcomponent
        ...
        <div class="form-group row form-item" >
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    @component('component/imgUpload',['plid'=>"backgroundImg",'parms'=>'{"type":"background","page_type":"2","uniqid":"'.$uniqid.'","m_type":"1"}','plurl'=>$apiurl.'/material/upload','name'=>'background','uploader'=>'bgoundUpload','direction'=>1,'browse_btn'=>'btn-bground','content_class'=>'innerBackground','img_format'=>'gif,jpg,jpeg,png','num'=>100,'value'=>$row['background']])
                    @endcomponent
        ...
        <div class="form-group row form-item" >
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    @component('component/imgUpload',['plid'=>"decorateImg",'parms'=>'{"type":"decorate","page_type":"2","uniqid":"'.$uniqid.'","m_type":"2"}','plurl'=>$apiurl.'/material/upload','name'=>'decorate','uploader'=>'decorateUpload','direction'=>1,'browse_btn'=>'btn-decorate','content_class'=>'innerDecorate','img_format'=>'gif,jpg,jpeg,png','num'=>100,'value'=>$row['decorate']])
                    @endcomponent
        ...
        <div class="form-group row form-item" >
            ...
            <div class="col-xs-12 col-sm-10">
                <div class="row">
                    @component('component/imgUpload',['plid'=>"frameImg",'parms'=>'{"type":"frame","page_type":"2","uniqid":"'.$uniqid.'","m_type":"3"}','plurl'=>$apiurl.'/material/upload','name'=>'frame','uploader'=>'frameUpload','direction'=>1,'browse_btn'=>'btn-frame','content_class'=>'innerFrame','img_format'=>'gif,jpg,jpeg,png','num'=>100,'value'=>$row['frame']])
                    @endcomponent

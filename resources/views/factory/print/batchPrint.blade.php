...
<div class="main-table" style="width: 100%;background-color: white;">
    ...
    <div style="background-color: white;width:85%;margin: 0 auto;padding-top: 30px">
        <select  class="form-control col-sm-2" id="prod_supplier_sn" name="prod_supplier_sn" style="font-size: 14px;margin-bottom: 10px;">
            @foreach($searchList as $k=>$v)
                <option value={{json_encode($v['sku_ids'])}} @if(json_encode($v['sku_ids']) == $skuIds) selected @endif >{{$v['prod_supplier_sn']}}</option>
            @endforeach
        </select>
        <div style="margin:0">
            @component('component.navOperateTab',['navlist'=>['all'=>'全部('.$statusCount[0].')','printed'=>'已打印('.$statusCount[1].')','not_printed'=>'未打印('.$statusCount[2].')'],'extendClass'=>"s_analy_tab",'defaut_key'=>$tab])@endcomponent
        </div>
        ...
        <table id="table" class="table table-bordered" width="100%">
            ...
            <tr class="nav_color">
                ...
                <th class="center" style="width: 10%">是否打印</th>
                <th class="center" style="width: 10%">供应商码</th>
            ...
            @foreach ($data as $item)
                ...
                <tr class="color">
                    <td colspan="7" style="vertical-align: middle;">
                        <input type="checkbox" data-value="{{$item['order_no']}}" data-code="{{$item['express_code']}}">
                        @if($item['error_msg'] != '')
                ...
                <tr class="pass">
                    ...
                    <td class="center">@if($item['is_print'] == PUBLIC_YES)已打印@else未打印@endif</td>
                    <td class="center">{{$item['prod_supplier_sn']}}</td>
                    <td class="center">{{$item['delivery_name']}}</td>
                    <td class="center" rowspan="1">
                        <div class="center-block">
                            <p><img style="width: 16px;height: 16px;margin-right: 5px;" src="/images/27.png"><span>{{$item['print_rcv_user']}}</span></p>
                            <p><img style="width: 16px;height: 16px;margin-right: 5px;" src="/images/28.png"><span>{{$item['print_rcv_phone']}}</span></p>
                            <p><img style="width: 16px;height: 16px;margin-right: 5px;" src="/images/30.png">
                                <span style="display: block;width: 99%">
                                    {{$item['receiver_info']['p']}}{{$item['receiver_info']['c']}}{{$item['receiver_info']['a']}}{{$item['print_rcv_address']}}
                                </span>
                            </p>
                        </div>
                    </td>
                    <td class="center">{{date('Y-m-d H:i:s',$item['order_create_time'])}}</td>
                    <td class="center" rowspan="1">
                        @if($item['is_print'] != PUBLIC_YES)
                            <p>
                                <button class="btn btn-primary btn-print" data-value="{{$item['order_no']}}" data-code="{{$item['express_code']}}">打印面单</button>
                            </p>
                        @endif
                    </td>
                </tr>
...
<script type="text/javascript">
    ...
    $(".btn-print").click(function () {
        ...
        delivery_type = $(this).attr("data-code")
        ...
        printFace(order_no,delivery_type)
    })
    ...
    $(".btn-print-all").click(function () {
        if(select_arr.length < 1){
            ...
            for (var index in select_arr){
                printFace(select_arr[index],select_arr_code[index])
            }
        }
    })

    function printFace(order_no,delivery_type) {
        $.ajax({
            type: 'POST',
            url: '/print/printData',
            dataType : 'json',
            data: {order_no:order_no,delivery_type:delivery_type},
            headers: {
    ...
    //下拉框
    $('#prod_supplier_sn').change(function () {
        var url = "/print/printSheet?sku="+$(this).val()+"&tab=all&limit=10";
    ...
    $(".main-table").delegate(".nav_status_btn","click",function () {
        ...
        //拼接跳转链接
        var url = "/print/printSheet?sku="+$('#prod_supplier_sn').val()+"&tab="+val+"&limit=10";
        window.location.href=url;

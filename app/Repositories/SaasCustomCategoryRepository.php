...
use App\Models\SaasCustomCategory;
 ...
 * @date:
 */
class SaasCustomCategoryRepository extends BaseRepository
{

    protected $isCache = false; //是否使用缓存

    public function __construct(SaasCustomCategory $model)
    {
        $this->model =$model;
    ...
    public function save($data)
    {
        ...
        if(empty($data['id'])) {
            ...
            unset($data['id']);
            $ret =$this->model->where('cus_cate_id',$priKeyValue)->update($data);
        }
    ...
    //树级无限级分类
    public function getTreeList($array=[],$pidname='cate_parent_id',$table_id='cus_cate_id',$name='cate_name')
    {
        $tree = new Tree();
        //组合无限极分类
    ...
    public function getParentList($array)
    {
        foreach ($array as $k=>$v){
            if ($v['cate_parent_id']!=0){
                $array[$k]['cate_parent_value'] = $this->model->where(['cus_cate_id' => $v['cate_parent_id']])->value('cate_name');
            }else{
    ...
    //获取类目名称
    public function getAllList($mid)
    {
        ...
        $array = [];
        foreach ($data as $k=>$v){
            $array[$v['cus_cate_id']] = $v['cate_name'];
        }
        return $array;
    }

    //获取父级分类id字符串
    public function getParentId($id,$mid)
    {
        $categoryArr = $this->model->where(['mch_id' => $mid])->get()->toArray();

        $tree = new Tree();
        //组合无限极分类
        $tree::instance()->init($categoryArr,'cate_parent_id','cus_cate_id');

...
class SaasUserRepository extends BaseRepository
{
    public function __construct(SaasUser $model)
    {
        $this->model =$model;
    }
    ...
    {
        //密码盐
        $data['salt'] = Helper::build();
        $data['user_birthday'] = !empty($data['user_birthday'])?strtotime($data['user_birthday']):null;
        $data['balance'] = !empty($data['balance'])?$data['balance']:0;
        $data['score'] = !empty($data['score'])?$data['score']:0;
        ...
        if(empty($data['user_id'])){
            ...
            $ret =$this->model->where('user_id',$priKeyValue)->update($data);
        }
        return $ret;
    }

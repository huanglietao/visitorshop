 ...
 * @date:
 */
class SaasUserMoneyRepository extends BaseRepository
{
    protected $isCache = false; //是否使用缓存

    public function __construct(SaasUserMoney $model)
    {
        $this->model =$model;
    }
    ...
    public function save($data)
    {
        if(empty($data['id'])) {
            ...
            $priKeyValue = $data['id'];
            unset($data['id']);
            $ret =$this->model->where('user_money_id',$priKeyValue)->update($data);
        }
        //判断是否需要更新缓存
         if (isset($this->isCache)&&$this->isCache === true){
             $table_name = $this->model->getTable();
             $redis = app('redis.connection');
             $data['user_money_id'] = $priKeyValue;
             //将数据写入缓存

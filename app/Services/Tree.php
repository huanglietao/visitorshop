...
class Tree
{
     ...
     *      )
     */
    public function init($arr = [], $pidname = NULL,$table_id=Null, $nbsp = NULL)
    {
        $this->arr = $arr;
        if (!is_null($pidname))
     ...
     * 省市区数组
    */
    public function inits($arr = [], $pidname = NULL, $nbsp = NULL)
    {
        $this->arr = $arr;
        if (!is_null($pidname))
            $this->pidname = $pidname;
    ...
    public function getChild($myid)
    {
        ...
        {
            if (!isset($value[$this->table_id]))
                continue;
            if ($value[$this->pidname] == $myid)
                $newarr[$value[$this->table_id]] = $value;
        }
        return $newarr;
    }
    ...
    public function getChildren($myid, $withself = FALSE)
    {
        ...
        foreach ($this->arr as $value)
        {
            ...
            {
                $newarr[] = $value;
                $newarr = array_merge($newarr, $this->getChildren($value[$this->table_id]));
            }
            else if ($withself && $value[$this->table_id] == $myid)
            {
                $newarr[] = $value;
            }
        }

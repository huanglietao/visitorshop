...
class Main
{
   ...
   /** Dai 加mid判断是哪个后台克隆的 */
    ...
    {
        if(empty($temp_id)) {
            Helper::EasyThrowException('50010', __FILE__.__LINE__);
        }

        //获取主表信息
        $main_info = SaasMainTemplates::where(['main_temp_id'=>$temp_id])->first();
        if(empty($main_info)) {
            Helper::EasyThrowException('50011', __FILE__.__LINE__);
        }
        ...
        $mainTemp->temp_tag = $main_info['temp_tag'];

        $mainTemp->save();
        $newId = $mainTemp->main_temp_id;//克隆后的新模板id
    ...
    public function addPhotoBookPages($params,$inner_id, $mid = 0)
    {
       ...
       // $is_page = 0;
        ...
        $is_turn = $specParam['size_is_cross'];//是否单双页
        ...
        for ($i=0; $i<$add_pages;$i++) {
            $startSort++;
            if ($is_turn == 1) { //自动生成对应的名称,例如 第1-2页名称规则
     ...
     * @param int $mid  商户id
     */
    public function addMainPhotoBookPages($params,$main_id,$mid = 0)
    {
        /**
        ...
        $data = [];
        $startSort = 0;
        $tempPage = config('goods.goods_type')[1];
        $ii = 0;
     ...
     * @param int $mid  商户id
     */
    public function addMainCalendarPages($params,$main_id,$mid = 0)
    {
    ...
    /**
     * 布局克隆
     * @param int $layout_id  布局id
     * @param int $mid 商户id
     * @return array
     */
    public function TempLayoutCopy($layout_id, $mid = 0)
    {
        if(empty($layout_id)) {
            Helper::EasyThrowException('50013', __FILE__.__LINE__);
        }

        //获取主表信息
        $main_info = SaasTemplatesLayout::where(['temp_layout_id'=>$layout_id])->first();
        if(empty($main_info)) {
            Helper::EasyThrowException('50014', __FILE__.__LINE__);
        }

        //复制主表记录
        $mainTemp = new SaasTemplatesLayout();
        $mainTemp->temp_layout_name = '克隆_'.$main_info['temp_layout_name'];
        $mainTemp->temp_layout_type = $main_info['temp_layout_type'];
        $mainTemp->goods_type_id = $main_info['goods_type_id'];
        $mainTemp->specifications_id = $main_info['specifications_id'];
        $mainTemp->layout_spec_style = $main_info['layout_spec_style'];
        $mainTemp->layout_dpi = $main_info['layout_dpi'];
        $mainTemp->layout_real_dpi = $main_info['layout_real_dpi'];
        $mainTemp->temp_layout_thumb = $main_info['temp_layout_thumb'];
        $mainTemp->layout_check_status = TEMPLATE_STATUS_DOING;
        $mainTemp->mch_id = $mid;
        $mainTemp->temp_layout_stage = $main_info['temp_layout_stage'];
        $mainTemp->layout_photo_nums = $main_info['layout_photo_nums'];
        $mainTemp->temp_layout_sort = ZERO;
        $mainTemp->created_at = time();
        $ret = $mainTemp->save();

...
use App\Http\Requests\Merchant\System\PaymentRequest;
use App\Repositories\SaasPaymentRepository;
use Illuminate\Http\Request;

/**
 * 项目说明 OMS系统 系统设置-支付配置
 * 详细说明 OMS系统 系统设置-支付配置，实现列表，添加，编辑，删除及组件结合
...
{
    protected $viewPath = 'merchant.system.payment';  //当前控制器所的view所在的目录
        ...
        try {
            if($request->ajax())
            {
                ...
                if($row['pay_id']){
                    ...
                    }else if($row['pay_class_name'] == 'wxpay'){
                ...
                //官方配置的支付方式
                $payNames = $this->repositories->getPayInfo();

                $htmlContents = $this->renderHtml($this->viewPath.'.'.$this->form, ['row' => $row,'payNames'=>$payNames]);

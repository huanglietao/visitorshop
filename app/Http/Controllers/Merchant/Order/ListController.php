...
{
    protected $viewPath = 'merchant.order.list';  //当前控制器所的view所在的目录
    protected $modules = 'sys';        //当前控制器所属模块
    protected $mchID;

    public function __construct(SaasOrdersRepository $ordersRepository,SaasDeliveryRepository $saasDeliveryRepository,
                                SaasOrderTagRepository $orderTagRepository,SaasCompoundQueueRepository $compoundQueueRepository,
    ...
    public function index()
    {
        ...
        $shop_list = $this->dmsAgentInfoRepository->getAgentList($this->mchID,1);
        return view("merchant.order.list.index",['pageLimit'=>$pageLimit,'chanelList'=>$chanel_list,'shopList'=>$shop_list]);
    }
    ...
    public function detail(Request $request)
    {
        ...
        return view("merchant.order.list.detail",['data'=>$data]);
    }
    ...
    public function cancelOrder(Request $request)
    {
        try{
            try{
                ...
                $order_id = $request->id;
                $res = $this->repositories->cancelOrder($order_id,session("admin")["oms_adm_username"],config('common.sys_abbreviation')['merchant'],session("admin")["oms_adm_id"]);
    ...
    public function changeInfo(Request $request)
    {
        ...
        try {
            if($request->post())
            {
                ...
                $htmlContents = $this->renderHtml('merchant.order.list.change_info',['data'=>$data]);
    ...
    public function changeDelivery(Request $request)
    {
        ...
        try {
            if($request->post())
            {
                ...
                $htmlContents = $this->renderHtml('merchant.order.list.change_delivery',['data'=>$data,'deliveryList'=>$delivery_list]);
    ...
    public function distribution(Request $request)
    {
        ...
        try {
            if($request->post())
            {
                ...
                $res = $this->repositories->distribution($order_id);
                if($res){
                    return $this->jsonSuccess('订单配货成功');
                }
            }else{
                $data = $this->repositories->productAndWork($order_id);
                $htmlContents = $this->renderHtml('merchant.order.list.distribution',['data'=>$data]);
                return $this->jsonSuccess(['html' => $htmlContents]);
            }
        } catch (CommonException $e) {
            return $this->jsonFailed($e->getMessage());
        ...
        try{
            try {
                if($request->post())
                {
                    ...
                    $res = $this->repositories->delivery($order_id,$param,session("admin")['oms_adm_username'],config('common.sys_abbreviation')['merchant']);
                    if($res){
                        \DB::commit();
                    ...
                    $htmlContents = $this->renderHtml('merchant.order.list.delivery',['productData'=>$prod_data,'deliveryList'=>$delivery_list,'orderInfo'=>$order_info]);
                    return $this->jsonSuccess(['html' => $htmlContents]);
                }
            } catch (\Exception $e) {
                \DB::rollBack();
...
//                    }
        ...
        try {
            try{
                if($request->post())
                {
                    ...
                    $res = $this->repositories->submitProduction($order_id);
                    if($res){
                        \DB::commit();
                    ...
                    $data = $this->repositories->productAndWork($order_id);
                    $htmlContents = $this->renderHtml('merchant.order.list.production',['data'=>$data]);
                    return $this->jsonSuccess(['html' => $htmlContents]);
                }
            } catch (\Exception $e) {
                \DB::rollBack();
    ...
    public function logistics(Request $request)
    {
        try{
            ...
            $htmlContents = $this->renderHtml('merchant.order.list.logistics',['data'=>$data]);
            return $this->jsonSuccess(['html' => $htmlContents]);

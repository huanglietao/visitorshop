...
use App\Http\Requests\Backend\Advertisement\AdlistRequest;
...
{
    protected $viewPath = 'backend.advertisement.adlist';  //当前控制器所的view所在的目录
    ...
    public function index()
    {
        ...
        return view('backend.advertisement.adlist.index',['adType'=> $adType,'channelList'=> $this->channelLists,'firstChannel'=>$this->firstChannel]);
    }
    ...
    protected function table(Request $request)
    {
        try {
            ...
            if(isset($inputs['status'])){
                $inputs['channel_id'] = $inputs['status'];
                unset($inputs['status']);
            }
            //获取分类标识
            ...
            //广告位置数据
            $adPosArr = $this->adPosRepositories->getAdPositionList();
    ...
    protected function form(Request $request)
    {
        try {
            if($request->ajax())
            {
                ...
                //广告位置数据
                if($row['ad_id']){
                    $adPosArr = $this->adPosRepositories->getAdPositionList(['channel_id'=>$row['channel_id']]);
                    $adPosList = Helper::ListToKV('ad_pos_id','ad_position',$adPosArr);
                }else{
                    if($channel == 'all'){
                        $adPosList = [];
                    }else{
                        $adPosArr = $this->adPosRepositories->getAdPositionList(['channel_id'=>$channel]);
                        $adPosList = Helper::ListToKV('ad_pos_id','ad_position',$adPosArr);
                    }
                }
   ...
   //添加/编辑操作
    ...
    public function getAdPosList(Request $request)
    {
        $post = $request->all();
        if($post){
            $adPosArr = $this->adPosRepositories->getAdPositionList(['channel_id'=>$post['id']]);
        }
        $posArray = [];
        if(!empty($adPosArr)){

...
(function(){
	...
	*/
	...
	//按日期得到节气
	function getDayTerm(year,month,day)
	{
       var arrTerm = getYearTerm(year);

	   
	   formatDay = formateDayD4(month-1,day);
	   return arrTerm[formatDay];
	}

	//获取某日的数据
	function getDayDate(year,month,day)
	{
		year = year.toString();
		month = month.toString();
		day = day.toString();
	    nl_formatDay = solarToLunar(year,month,day);
			// 今天是本月第几周
		sDObj = new Date(year, month-1, day);
		var nowWeek = sDObj.getDay();
		//alert(parseInt(nowWeek));
		var witchWeek = Math.ceil((parseInt(day) + parseInt(nowWeek)) / 7);
		if(month.length==1)
		{
			month = '0'+month;
		}
		var week_index = month+witchWeek+nowWeek;
		//节日显示设置
		nl_formatDay['display'] = 0;
		if(nl_formatDay['solarFestival'].indexOf("|#")>0)
		{
			var arr=nl_formatDay['solarFestival'].split("|#");
			nl_formatDay['solarFestival'] = arr[0];
			nl_formatDay['display'] = 1;
		}
		
		//农历节日固定为1
		if(nl_formatDay['lunarFestival'])
		{
			nl_formatDay['display'] = 1;
		}

		if(typeof(wFtv[week_index])!= "undefined")
		{
			nl_formatDay['solarFestival'] = wFtv[week_index];
			nl_formatDay['display'] = 1;
		}
	
	    return JSON.stringify(nl_formatDay);
	}
	...
	function solarToLunar(_year,_month,_day){
		...
		var res = {
			...
			term : termList[formateDayD4(month,day)]?termList[formateDayD4(month,day)]:'',
			...
			solarFestival : solarFestival[formateDayD4(month,day)]?solarFestival[formateDayD4(month,day)]:'',
			lunarFestival : lunarFtv?lunarFtv:''
		};
	...
	var LunarCalendar = {
		...
		getYearTerm:getYearTerm,
		getDayTerm:getDayTerm,

...
$(function(){
    ...
    //         '<input type="text" class="form-control" width="100%" id="order_no" name="order_no[]"  placeholder="天猫/淘宝/京东/订单号" autocomplete="off" value=""  onkeyup="this.value=this.value.replace(/[^\\d]/g,\'\')" onafterpaste="this.value=this.value.replace(/[^\\d]/g,\'\') "/>' +
    //         '</div> ' +
    ...
    $("body").delegate("#tongbu",'click',function () {
        ...
        if(order_no==""){
            layer.msg("请输入要同步的订单号");
            return;
        }

        // var orderNo_arr = [];
        // //多个订单号存到数组中
        // for(var i=0;i<no_input.length;i++){
        //     var no = no_input.eq(i).val();
        //     if(!no){
        //         layer.msg("请输入要同步的订单号!");
        //         return;
        //     }else{
            ...
            success : function (data) {
                if(data.status==404&&data.success=='false'){
                    ...
                    for ( var r = 0; r <obj_orders.length; r++){
                        ...
                        goods_html +='<td style="vertical-align: middle;width: 10.66%;">'+tb_order_status[outer_order_data['status']]+'</td>';

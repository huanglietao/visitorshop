$(function(){
    ...
    $(".btn-job-export").click(function () {
        ...
        var str = JSON.stringify(obj);
        window.location.href = '/order/service/export/'+str
    })

    //换货单商品数量
    $("body").delegate(".exchange_num","blur",function () {
        var num = $(this).val()
        var pattern = /^([0]|[1-9][0-9]*)$/

        if(num < 1 || pattern.test(num) == false){
            layerMessage('数量不能小于1')
        }

        var arr_val = JSON.parse($(this).next().val())
        for( var key in arr_val ){
            arr_val[key] = parseInt(num)
        }
        $(this).next().val(JSON.stringify(arr_val))
    })

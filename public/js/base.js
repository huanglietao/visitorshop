...
ready.run = function(_$){
  ...
  layer.open = function(deliver){
    ...
    layer.commonopen = function (html,title,deliver) {
        ...
        {
          console.log(deliver);
            deliver.area = deliver.area?deliver.area:['850px', '600px'];
            deliver.skin = deliver.skin?deliver.skin:"layui-layer-fast";
            deliver.resize = deliver.resize?deliver.resize:false;
...
$(function(){
    ...
    //==================分页相关js开始 =========//
    $("#pages-item").on('change', function(){ //跳到第xx页
        var pages = $(this).val();
        loadTable({
            page :pages,
            limit : $('#pages-limit').val()
        });

    });

    $("#pages-limit").on('change', function(){ //设置分页数
        loadTable({
            limit : $(this).val()
        });
    });
    //首页
    $(".paginate").find('.pages').on('click', function(){
    ...
    //加载记录列表
    function loadTableList(url, search){
        $(".tbl-content").html('');
        $.ajax({
            url :url,
            type: 'POST',
   ...
   //操作提示的js
    ...
    $('#main').delegate(".btn-dialog",'click',function () {
        ...
        $.ajax({
            ...
            success : function(data){
                layer.commonopen(data.html,title,params)
            },
            error : function(){
    ...
    //操作成功提示
    tip_success = function (text,interval) {
    ...
    $("body").delegate(".btn-submit","click",function () {
        ...
        var layerIndex = $(this).parents(".layui-layer").attr("times");
...
//成功提示
    $(".search-more-hide").on('click', function(){
        $(".search-open").hide();
        $(".btn-search").show();

...
$(function(){
  ...
  //点击添加或编辑按钮触发页面跳转
  $("body").delegate(".btn-face-temp",'click',function () {
    ...
    $.ajax({
      ...
      success : function (data) {
        if(data.success == 'true'){
          tip_success('/templatecenter/face');
        }else {
      ...
      $.ajax({
        ...
        success : function(data){
          if(data.status==200){
            ...
            //规格参数的data-url变化
            $('#spec-params').attr('data-url', '/templatecenter/face/specdetail?id='+init);
            getBackPx(init);
          }
  ...
  {
    $.ajax({
      url : '/templatecenter/face/getBackPx',
      type : 'POST',
      data : {sizeId:id},
      dataType : 'JSON',
      headers: {
  ...
  $("body").delegate('.check_status','change',function () {
    ...
    var status = $(this).val();
    $.ajax({
      url : '/templatecenter/face/checkstatus',
      type: 'POST',
      data:{
        status:status,
        id:id,

...
$(function(){
    ...
    $(document).ready(function () {
        var cate_id = $(".s_analy_tab").attr("data-val");
        //加入加载标示
    ...
    function loadtable(cate_id,limit,curPage) {
        ...
        $.ajax({
            url : url,
            type: 'post',
            data : {"search":search,"limit":limit,"curPage":curPage},
            dataType : 'JSON',
            headers: {
    ...
    $("#download").click(function () {
        ...
        var search = $("#searchID input[type='text']").val();
        if(cate_id==1){
            location.href = "/reconciliation/export?search="+search;
        }else{
            location.href = "/orders/export?search="+search;
        }
    ...
    //======================数据导入js开始 =============//
    tips = function (filename,total,num) {
        if(num===2){
    ...
    $("body").delegate("#sure-comfirm","click",function () {
